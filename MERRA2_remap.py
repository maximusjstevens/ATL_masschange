...
class MERRA_remap:
    ...
    def M2remap(self,icesheet,LLbounds,out_path,YY,calc_melt=True,freq='1D'):
        '''
        ...
        out_full_dsALL = pathlib.Path(out_path,f'netCDF/{freq_name}',dsALL_fn) 
        
        if os.path.exists(out_full_dsALL): # has been subsetted.
            dsALL = xr.open_dataset(out_full_dsALL)
        else:
        ...
        ### (do not need to save the non-con fields in their own netcdf, as they are already in the dsALL
        
        if not os.path.exists(out_full_noc_rem):
            ###############
            ### non-conservative fields (bilinear and lapse rate)        
            ### Load ATL15
            if icesheet=='GrIS':
            ...
            ### bilinear - just the few that don't get mapped otherwise (conservative or with the T/lapse rate calculation)
            ds_M2_10k = ((dsALL[['ALBEDO','HLML','EMIS_eff']].rename({'lat':'y','lon':'x'}).rio.write_crs(4326,inplace=True).rio.set_spatial_dims(x_dim="x", y_dim="y",
                        inplace=True).rio.write_coordinate_system(inplace=True)).rio.reproject_match(atl15_10k,resampling=Resampling.bilinear))
            
            ### dataset
            ds_M2_10k_ALL = ((dsALL.rename({'lat':'y','lon':'x'}).rio.write_crs(4326,inplace=True).rio.set_spatial_dims(x_dim="x", y_dim="y",
                        inplace=True).rio.write_coordinate_system(inplace=True)).rio.reproject_match(atl15_10k,resampling=Resampling.bilinear))
            
            ############################################
            ### temperature remapping using lapse rate
            ############################################
            ds_M2 = (dsALL[['TS','TScalc','T2M']].rio.write_crs(4326,inplace=True).rio.set_spatial_dims(x_dim="lon", y_dim="lat",
                    inplace=True).rio.write_coordinate_system(inplace=True))
            
            ### open data set that is ATL14 sampled to ATL15 10k grid using reproject match (average)
            ### This is done in ATL14_repmatch.py
            ### (older version used using true ATL14 data and xr coarsen function - did not work right?)
            ...
            LON,LAT = np.meshgrid(ds_M2.lon.values,ds_M2.lat.values)
            ...
            # z_15 = ATL14.h.interp_like(atl15_10k)
            # atl15_10k['z'] = z_15
            atl15_10k['z'] = ATL14
            ...
            for vv in ['TS','T2M','TScalc']:
                ...
                ATL_T = np.zeros((len(ATL15_xyz_rav),len(ds_test.time)))
    
                tttall = (ds_test.stack(xy=['lat','lon']).values).T
                for kk in range(len(ATL15_xyz_rav)):
                    iiis = kdi[kk]
                    if len(iiis)==0:
                        ided = np.nan * np.ones(len(ds_test.time))
                    else:
                        ttt = tttall[iiis]
                ...
                ds_M2_10k[f'{vv}_i'] = (['y','x','time'],ATL_T.reshape(atlX_ro,atlX_co,len(ds_test.time)))
                ds_M2_10k[f'{vv}_i'] = ds_M2_10k[f'{vv}_i'].T.astype('float32')
    
                ds_M2_10k_ALL[f'{vv}_i'] = (['y','x','time'],ATL_T.reshape(atlX_ro,atlX_co,len(ds_test.time)))
                ds_M2_10k_ALL[f'{vv}_i'] = ds_M2_10k[f'{vv}_i'].T.astype('float32')
                ## end for vv in [] 
            ...
            save_bilinear = False
            if save_bilinear:
                ds_M2_10k_ALL.to_netcdf(out_full_bilinear_rem) # save just the non-conserved, resampled variables
                print('Bilinear interp saved',flush=True)

        else:
            ds_M2_10k = xr.open_dataset(out_full_noc_rem)
        ##############
        
        if os.path.exists(out_full_con_rem): # need to remove because a previous failed run will have created an incomplete netCDF.
            try:
...
if __name__ == '__main__':
    ...
    if 'cdo' not in str(pp):
        print('cdo not loaded. Remapping will not happen. exiting.')
        sys.exit()
    ...
    Mc = MERRA_concat()
    Mc.M2combine(icesheet,LLbounds,out_path,YY,freq=freq)
    print(f'Done with making yearly files with {freq} resolution.')
    # print('Now making zarr.')
    # MZ = make_zarr()
    # MZ.make_zarr(icesheet,LLbounds,out_path,freq=freq)